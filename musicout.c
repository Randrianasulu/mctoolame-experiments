...
/********************************************************************
*
*        This part contains the MPEG I / II decoder for Layer II.
*
*********************************************************************/
...
{				/* R.S. 7 channels for ML */
  ...
  //MFC  char frame_name[MAX_NAME_SIZE];
  char lfe_file_name[MAX_NAME_SIZE];
  char t[50];
  int need_aiff;
  int m, print_out = 0;
  int ch_start, lfe_init = 1;
  int tca_log = 0, dynx_log = 0, bits_log = 0, rate_log = 0, scfsi_log =
    0, any_log = 0;
...
/************** interpret CL Args *****************/
  ...
  while (!end_bs (&bs_mpg)) {
    ...
    if (info.lay == 2) {
      ...
      II_decode_scale (&bs_mpg, &fr_ps, scfsi, bit_alloc, scale_index,
		       &ch_start, &channels, scfsi_log);
	...
	if (fr_ps.header->ext_bit_stream_present) {
	  ...
	  if (!sync) {
	    fprintf (stderr,
		     "Extension frame cannot be located, end of input stream\n");
	    exit (1);
	  }
    ...
    if (rate_log)
      printf ("frame: %5li bitrate: %8.3f kbps\n", frameNum,
	      frameBits * S_freq / samplesPerFrame);
    else if (mpeg >= 2) {
	...
	if (fr_ps.header->no_of_multi_lingual_ch > 7) {
	  ...
	  printf
	    ("            This decoder supports only upto 7 channels.\n\n");
	}
	printf ("There are %d multilingual channels, coded at ",
		fr_ps.header->no_of_multi_lingual_ch);
	...
	if (info.lfe)
      ...
      /* Achtung: fr_ps.jsbound = 27; wird in Abh�ngikeit von fs gesetzt */
      ch_start = stereo;
      channels = stereo + mc_channel;

      /* decoding bitallocation's */

      II_decode_bitalloc_mc (&bs_mc, &fr_ps, bit_alloc, &ch_start, &channels,
			     bits_log);
      II_decode_scale (&bs_mc, &fr_ps, scfsi, bit_alloc, scale_index, &ch_start,
		       &channels, scfsi_log);
	...
	if (new_crc != old_crc) {
      ...
      clip = 0;
      for (i = 0; i < 12; i++) {
	II_buffer_sample_mc (&bs_mc, &fr_ps, (*sample), bit_alloc, ch_start,
			     channels, i);
	if (!any_log) {
	  II_dequantize_sample_mc ((*sample), bit_alloc, *fraction, &fr_ps,
				   ch_start, channels, &i);
	  II_denormalize_sample_mc (*fraction, scale_index, &fr_ps, i >> 2,
				    ch_start, channels, &i);
	}
      }
	...
	if (lfe_init) {
	  ...
	  if (need_aiff) {
	    ...
	    if (aiff_seek_to_sound_data (lfe) == -1) {
	      fprintf (stderr,
		       "Could not seek to LFE PCM sound data in \"%s\".\n",
		       lfe_file_name);
	      exit (1);
	    }
	  }
...
/* JMZ 09/03/1995 Multilingual , WtK 7/8/95 */
	...
	II_decode_scale_ml (&bs_mc, &fr_ps, scfsi_ml, bit_alloc_ml,
			    scale_index_ml, &m);
	...
	for (i = 0; i < ((info.multi_lingual_fs == 0) ? 12 : 6); i++) {	/* ngr : n_of_granules loop */
	  ...
	  if (!any_log) {
	    II_dequantize_sample_ml ((*sample_ml), bit_alloc_ml, *fraction_ml,
				     &fr_ps, &m, &i);
...
/*********************************************************************/
...
#ifdef Augmentation_7ch
      if (mpeg == 3)
	if (stereo != 2 || fr_ps.header->surround != 2
	...
	II_decode_bitalloc_aug (&bs_mc, &fr_ps, bit_alloc, &ch_start, &channels,
				bits_log);
	II_decode_scale (&bs_mc, &fr_ps, scfsi, bit_alloc, scale_index,
			 &ch_start, &channels, scfsi_log);
	...
	for (i = 0; i < 12; i++) {
	  ...
	  if (!any_log) {
	    II_dequantize_sample_aug ((*sample), bit_alloc, *fraction, &fr_ps,
				      &i);
	    II_denormalize_sample_aug (*fraction, scale_index, &fr_ps, i >> 2,
				       &i);
	  }
	}
      }				/* endif mpeg == 3 */
...
/*********************Ausgabe***************************************************/
	...
	for (hu = 0; hu < 8; ++hu) {
	  ...
	  for (ho = 0; ho < 4; ho++) {
	    fprintf (fp4, "mc_predsi[%d][%d] = %d  ", hu, ho,
		     info.mc_predsi[hu][ho]);
	    fprintf (fp4, "mc_delay[%d][%d] = %d  ", hu, ho,
		     info.mc_delay_comp[hu][ho]);
	    fprintf (fp4, " \n");
	    for (hi = 0; hi < 3; hi++)
	      fprintf (fp4, "pred_coeff[%d][%d][%d] = %d ", hu, ho, hi,
		       info.mc_pred_coeff[hu][ho][hi]);
	    fprintf (fp4, " \n");
	  }
	}
#endif
	...
	if (info.surround == 3)
	  for (ho = 0; ho < 12; ho++) {
	    fprintf (fp5, "dyn_second_stereo[%d] = %d   ", ho,
		     info.dyn_second_stereo[ho]);
	    if ((ho + 1) % 4 == 0)
	      fprintf (fp5, "\n");
	  }
      }
	...
	for (hi = 0; hi < channels; hi++) {
	  ...
	  for (ho = 0; ho < layer_I_frames; ho++) {
	    fprintf (fp1, "SCF[%d][%d][%02d] = %d   ", hi, ho, hu,
		     scale_index[hi][ho][hu]);
	  }
	}
	for (hi = 0; hi < info.no_of_multi_lingual_ch; hi++) {
	  fprintf (fp1, "\n");
	  /* 960315 added FDB Multi-Lingual bitalloc */
	  fprintf (fp2, "BAL_ML[%d][%02d] = %d   ", hi, hu,
		   bit_alloc_ml[hi][hu]);
	  ...
	  for (ho = 0; ho < 3; ho++)
	    fprintf (fp1, "SCF_ML[%d][%d][%02d] = %d   ", hi, ho, hu,
		     scale_index_ml[hi][ho][hu]);
	}
	fprintf (fp1, "\n");
      }
...
/*******************************************************************************/
 ...
 /****************************************************************/
...
#ifdef  MACINTOSH
  if (need_aiff)
    set_mac_file_attr (decoded_file_name, VOL_REF_NUM,
  ...
  if (fr_ps.header->no_of_multi_lingual_ch > 0) {
    ...
    if (need_aiff)
      printf
	("Multilingual file has been written with AIFF header information\n");
  }
...
{				/* print syntax & exit */
  ...
  fprintf (stderr,
	   " -a       write a RAW data sound file  (default: write an AIFF sound file)\n");
  fprintf (stderr,
	   " -b       write alloc bits per subband to stdout (default: off)\n");
  fprintf (stderr, " -d       write dynx modes to stdout (default: off)\n");
  fprintf (stderr,
	   " -h       bitstreams contain a 2304 byte header  (default: no header)\n");
...
#ifdef Augmentation_7ch
  fprintf (stderr,
	   " -n       encoded bitstream is a MPEG 2 Layer-2 file with 7.1 augmentation\n");
  /* default: decode a MPEG 2 Layer-2 bitstream */
#endif
  fprintf (stderr,
