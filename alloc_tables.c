...
int pick_table (frame_params *fr)
{
  ...
  //br = bitrate[fr->header->version][2-1][fr->header->bitrate_index] / fr->stereo;
  //sfrq = s_freq[fr->header->version][fr->header->sampling_frequency];
  ...
  if (fr->tab_num != table)
  {
    ...
    switch(table)
    {
      ...
      case 4:  fr->alloc = &al_table_4; break;
...
void mc_pick_table (frame_params * fr_ps)
{
  ...
  /* MFC March 2003
     From ISO13818 Section 2.5.2.12
     "Table B.2a (table 0) shall be used if sfrq = 48kHz, Table B.2b (table 1) 
      shall be used if sfrq 44.1kHz or 32kHz, regardless of bitrate" */
  if (sfrq == 48)
    tablemc = 0;
  else
    tablemc = 1;
  ...
  if (fr_ps->header->no_of_multi_lingual_ch > 0) {
    /* ML channels are at same frequency */
    if (fr_ps->header->multi_lingual_fs == 0) {
      ...
      /* ML channels are at half-frequency of main channels - LSF */
      tableml = 4;
